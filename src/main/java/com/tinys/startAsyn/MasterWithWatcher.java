...
/**
 * 比Master类多了一个监听
 * 会在master节点发生变化时,得到通知并执行相应的竞争
 */
public class MasterWithWatcher implements Watcher{
    ...
    public MasterWithWatcher(String hostport) {
    ...
    Watcher masterExistsWatcher = new Watcher() {
        public void process(WatchedEvent watchedEvent) {
            ...
            if (watchedEvent.getType() == Event.EventType.NodeDeleted){
                //删除的路径是master
                if (watchedEvent.getPath().equals(MASTER)){
                    //竞争选主
                    runForMaster();
                }
            }
    ...
    //回调函数是调用后的异步通知,Watch是会话存活时间内的持续监听
    ...
    public static  void main(String[] args) throws Exception {
        MasterWithWatcher master = new MasterWithWatcher("192.168.198.128:2222");
        master.startZk();
        master.runForMaster();
        Thread.sleep(15000);
        master.stopZk();
